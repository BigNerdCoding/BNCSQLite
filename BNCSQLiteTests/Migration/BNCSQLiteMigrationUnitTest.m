...
- (void)testVersion1_to_Version2 {
    ...
    [[v1Table dbConnect] executeSQL:sql bind:nil rowHandle:^(BNCSQLiteDatabaseStatement *statement, uint64_t rowNum) {
    ...
    [[v2Table dbConnect] executeSQL:sql bind:nil rowHandle:^(BNCSQLiteDatabaseStatement *statement, uint64_t rowNum) {
        NSDictionary *dic = [statement takeAllColumn];
        [allColumn addObject:dic];
    } error:&error];
...
- (void)testVersion1_to_Version3 {
    ...
    allColumn = [NSMutableArray array];
    [[v3Table dbConnect] executeSQL:sql bind:nil rowHandle:^(BNCSQLiteDatabaseStatement *statement, uint64_t rowNum) {
        NSDictionary *dic = [statement takeAllColumn];
        [allColumn addObject:dic];
    } error:&error];
...
- (void)testVersion1_to_Version4 {
    ...
    allColumn = [NSMutableArray array];
    [[v4Table dbConnect] executeSQL:sql bind:nil rowHandle:^(BNCSQLiteDatabaseStatement *statement, uint64_t rowNum) {
        NSDictionary *dic = [statement takeAllColumn];
        [allColumn addObject:dic];
    } error:&error];
